"""
LinkedIn Profile Accessor Monitor - Approval Workflow Monitor

Monitors the Approved/ folder for LinkedIn profile analysis requests
and generates reports when approved files are detected.

This monitor works with the existing approval workflow system.
"""
...
class LinkedInAccessorMonitor:
    """
    Monitor for LinkedIn profile accessor approval workflow.

    Watches the Approved/ folder for LINKEDIN_PROFILE_ANALYSIS_*.md files
    and processes them when detected.

    Usage:
        monitor = LinkedInAccessorMonitor(vault_path="AI_Employee_Vault")
    ...
    def _log_audit_action(
        ...
        try:
            self.audit_logger.log_action(
                ...
                target="linkedin_accessor_monitor",
    ...
    async def check_for_approved_files(self) -> list[Path]:
        """
        Check for approved profile analysis files.
        ...
        # Find all LINKEDIN_PROFILE_ANALYSIS_*.md files in Approved/
        approved_files = list(self.approved_dir.glob("LINKEDIN_PROFILE_ANALYSIS_*.md"))
    ...
    async def process_approved_file(self, file_path: Path) -> bool:
        """
        Process an approved profile analysis file.
        ...
        try:
            ...
            if file_type != "linkedin_profile_analysis":
            ...
            # The file is already an analysis report, so we just need to:
            # 1. Mark it as processed
            # 2. Optionally trigger the Profile Builder if improvements are requested

            # Mark as processed
            self.processed_files.add(str(file_path))

            # Move to Done/
            await self._move_to_done(file_path, summary="Profile analysis completed")
    ...
    async def run(self) -> None:
        ...
        print(f"LinkedIn Profile Accessor Monitor started")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="LinkedIn Profile Accessor Monitor")
    ...
    monitor = LinkedInAccessorMonitor(
